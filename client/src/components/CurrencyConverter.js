...
export default function CurrencyConverter(props) {
  ...
  const cryptoList = state[0].data.map((crypto) => {
    return (
      ...
      >
        {crypto.name}
        <img src={crypto.image} alt="crypto" width="30"></img>
      </MenuItem>
    );
  });
  ...
  return (
    <>
      ...
      >
        <Typography variant="h4" fontFamily={"Pacifico"} textAlign={"center"}>
            ...
            <Chip
              ...
              label={
                <Typography
                  variant="h6"
                  textAlign={"center"}
                >{`${number} ${primary.name} = ${result} ${secondary.name}`}</Typography>
              }
            ></Chip>
          ) : null}
        </Box>
