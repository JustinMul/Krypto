...
const MarketCrypto = (props) => {
  ...
  return (
    ...
    <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 }}} style={{backgroundColor: "rgb(100, 220, 250)"}}>
      ...
      <TableCell align="left">
        <Link to={`/crypto/${cleanedUrl}`} 
          style={{ 
            textDecoration: 'none', 
            color: (Math.round(props.price_change_percentage_24h) > 0) ? 
            "green" : "red"
          }}>
          <div className='shiftdown'>
            {Math.round(props.price_change_percentage_24h)}% 
            {(Math.round(props.price_change_percentage_24h) > 0) ?
            <div className='flexdown'><FileUploadIcon/></div>
            : 
            <div className='flexdown'><FileDownloadIcon/> </div>}
          </div>
        </Link>
      </TableCell>
      <TableCell align="left">
      ...
      { 
        ...
        <TableCell align="center">
          <Button aria-label="like"  
            onClick={() => 
              props.setFav([props.id, props.image])
            }>
            <FavoriteIcon 
              style={{ color: red[500] }}
            />
