...
app.put("/insert-watchlist", (req, res) => {
  ...
  db.query(
    `INSERT INTO watchlists (user_email, crypto_id, user_email_cyrpto_id)
  VALUES ($1, $2, $3) RETURNING *;`,
    [req.body.user.email, req.body.id, `${req.body.user.email}${req.body.id}`]
  )
...
app.put("/user-delete", (req, res) => {
  let response = req.body.data;
  console.log("req.body for fav: ", req.body);
  db.query(`DELETE FROM watchlists WHERE user_email = $1 and crypto_id = $2;`, [
    req.body.user.email,
    req.body.data,
  ])
    .then((res) => res.send(res))
